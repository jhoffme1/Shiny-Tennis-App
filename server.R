server <- function(input, output) {
    ...
    url <- a("LinkedIn", href="Linkedin Link")
    url2 <- a("GitHub", href="Githib link")
    ...
    output$closingtext <- renderUI({
        HTML(paste('<br/><br/> "Text to write in". "More Text for conclusions"
               <p align="right"><i>"A Quote to add if you want to."))
...
output$First.Round.exp = rendorPlot(
...
output$Second.Round.exp = rendorPlot( 
...
output$Third.Round.exp = rendorPlot(  
...
output$Fourth.Round.exp = rendorPlot(
...
output$Quarterfinals.exp = rendorPlot(
...
output$SemiFinals.exp = rendorPlot(
...
output$The.Final.exp = rendorPlot(
